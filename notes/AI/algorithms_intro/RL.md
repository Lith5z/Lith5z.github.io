...
得到T和R，就可以转换为之前所说的MDP问题，通过价值迭代等方法来求策略pi
可以根据这个策略继续进行探索，反复进行
...
在**固定的策略**下，不断进行游戏，统计(state, value)，目标是得到**状态的价值**，而非求出T R然后再求解MDP（基于模型）
但是单通过价值得不到最优策略（结尾部分会解释）
...
时间差分学习是一种可以在不知道T R的情况下，用类似价值评估得到价值的方法
...
（一开始$$ V^{\pi}(s') $$为0，直接带入；之后发生了更新就带入更新后的值）
...
这里的alpha叫学习率
$$(1 - \alpha) < 1$$，越早得到的sample占最后$$ V^{\pi}(s') $$的权重逐渐缩小，越后面获得的样本越重要
...
所以这个方法是不完整的，只能得到价值而得不到策略
（作为对比，策略迭代算法同时进行了策略评估和提取）
...
**Q-Value Iteration** 基于模型的利用
类似价值迭代，从Q价值出发，需要已知转移模型T和奖励模型R
通常设初始化$$Q_0(s,a) = 0$$
...
最终，**最优策略**由收敛后的Q值导出：$$\pi^*(s) = \arg\max_a Q(s,a)$$
> 特别的，如果知道是Q*，那么可以直接知道策略，因为qstate就是action后的价值（MDP笔记）

另外，公式中的max保证了无论本次行动是什么，Q值都是根据目前为止最优的行动更新的，所以Q-Learning是不需要策略的 off-policy learning
...
显而易见更好的做法是，对于已经探索过足够次数的地方，就不在选择随机探索
...
之前提到的都是精确的方法，进行Q-Learning的时候，要维护所有的Q-State，总数量是状态数×行动数，这可能完全无法容纳
...
这样就通过少量的特征权重数据概括了大量的Q-State，学习速度会快很多
但是如果特征太少或者不恰当，很可能让完全不相似的情况有一样的特征值，就没办法区分；如果特征太多，样本又不够多，可能会发生过拟合

## Policy Search 策略搜索

之前针对Q价值进行学习，但其实数值本身不重要，重要的是Q价值的排序反映的action
比如吃豆人项目里，近似Q值可能非常离谱（比如在绝境给出+100），但只要它在危险动作上给的分数更低，在吃豆动作上给的分数更高，那给出的策略就是对的
为何不直接去学那个最大化reward的策略本身？这就是策略搜素

最简单的策略搜索，先从一个已经学习好的Q函数和策略出发，爬山法微调权重，再直接执行策略，如果效果更好就保留
